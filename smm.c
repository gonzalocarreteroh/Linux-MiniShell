...
void *mm_malloc(size_t size)
{
    ...
    while (cur < cur_heap_break)
    {
        ...
        // Not found free block that fits yet, so we look in the next metadata
        cur += meta_data_size + md->size;
    }

    // No free block that fits is found
    ...
    return cur_heap_break + meta_data_size; // you should return a suitable address here
}
...
void mm_free(void *p)
{
    ...
    {
        struct MetaData *md = (struct MetaData *)cur;
        if (cur + meta_data_size == p)
        {
            md->status = META_DATA_STATUS_FREE;
            return;
        }

        // We look to see if the pointer is pointing to the next block
        cur += meta_data_size + md->size;
    }

    // We don't do anything if the pointer does not point to a valid block
}
    ...
    while (cur < cur_heap_break)
    {
        ...
        if (md->status == META_DATA_STATUS_FREE)
        {
            ...
            if (next < cur_heap_break)
            {
                ...
                {
                    // We increase the size of the current block but remain in it as
                    // there might also be more free blocks following
